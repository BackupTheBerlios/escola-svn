...
import br.ufg.inf.apsi.escola.componentes.ca.servico.local.LocalUsuarioService;
...
public class LocalUsuarioServiceTest {
	...
	public void testAlterarValidade() {
		...
		try {
			context.checking(new Expectations() {
				{
					one(usuarioNegocio).alterarValidade(uid, cal);
				}
			});

			localUsuarioService.alterarValidade(100, cal);
	...
	public void testAlterarDisponibilidade() {
		...
		try {
			context.checking(new Expectations() {
				{
					one(usuarioNegocio).alterarDisponibilidade(uid,
							disponibilidade);
				}
			});

			localUsuarioService.alterarDisponibilidade(100, true);
	...
	public void testRemoverUsuarios() {
		...
		try {
			context.checking(new Expectations() {
				{
					one(usuarioNegocio).removerUsuario(uid);
				}
			});

			localUsuarioService.removerUsuario(100);
